...
> **[jarrodwatts/claude-hud](https://github.com/jarrodwatts/claude-hud) 的个人 fork。** 与上游的三处差异：(1) 会话 token 统计显示在第一行末尾，而非单独一行；(2) 默认将 `environment` + `tools` 合并为一行；(3) 默认开启活动行（tools、agents、todos），新设备安装后零配置即可显示；config counts、speed、session name、session tokens 保持关闭，第一行止于时钟/风格标签。完整说明（改动历史、新设备安装步骤、上游版本合并流程）见 [FORK-GUIDE.md](FORK-GUIDE.md)（英文）。
